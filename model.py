...
def IoU_loss(boxA,boxB):
    iou=[]
    for i in range(N):
        xA = K.maximum(boxA[i][0], boxB[i][0])
        yA = K.maximum(boxA[i][1], boxB[i][1])
        xB = K.minimum(boxA[i][2], boxB[i][2])
        yB = K.minimum(boxA[i][3], boxB[i][3])
    ...
    # rectangles
        boxAArea = (boxA[i][2] - boxA[i][0]) * (boxA[i][3] - boxA[i][1])
        boxBArea = (boxB[i][2] - boxB[i][0]) * (boxB[i][3] - boxB[i][1])

    # compute the intersection over union by taking the intersection
    # area and dividing it by the sum of prediction + ground-truth
    # areas - the interesection area
        iou_each = interArea / (boxAArea + boxBArea - interArea)
        iou.append(iou_each)
    loss = tf.stack(iou)
    return (-loss)       
'''    
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    # Smooth-L1 Loss
    smooth_loss = K.switch(tf.size(target_bbox) > 0,
                    smooth_l1_loss(y_true=target_bbox, y_pred=pred_bbox) ,
                    tf.constant(0.0)) 
    # IoU loss
    iou_loss = IoU_loss(target_bbox, pred_bbox)
    smooth_loss = K.mean(smooth_loss)
    loss = K.reshape(smooth_loss, [1,1]) #- K.mean(iou_loss), [1, 1])
    return loss
...
def mrcnn_mask_edge_loss_graph(y_pred, y_true, edge_filters, smoothing_predictions, smoothing_gt, norm, weight_entropy, weight_factor):
    ...
    if edge_filters == ["canny"]:
        y_pred = tf.expand_dims(y_pred, axis=0)
        y_pred_edges = TF_Canny(y_pred, return_raw_edges=True)
        y_true = tf.expand_dims(y_true, axis=0)
        y_true_edges = TF_Canny(y_true, return_raw_edges=True)
    ...
    '''    
    y_true_nocs_edges_x = y_true[:,:,:,0] & y_true_edges
    y_pred_nocs_edges_x = y_pred[:,:,:,0][tf.math.equal(y_pred_edges,tf.constant(1.0))]
    y_true_nocs_edges_y = y_true[:,:,:,1][tf.math.equal(y_true_edges,tf.constant(1.0))]
    y_pred_nocs_edges_y = y_pred[:,:,:,1][tf.math.equal(y_pred_edges,tf.constant(1.0))]
    y_true_nocs_edges_z = y_true[:,:,:,2][tf.math.equal(y_true_edges,tf.constant(1.0))]
    y_pred_nocs_edges_z = y_pred[:,:,:,2][tf.math.equal(y_pred_edges,tf.constant(1.0))]
    
    y_true_nocs_edges = tf.stack(y_true_nocs_edges_x, y_true_nocs_edges_y, y_true_nocs_edges_z)
    y_pred_nocs_edges = tf.stack(y_pred_nocs_edges_x, y_pred_nocs_edges_y, y_pred_nocs_edges_z)
    '''
    diff = K.abs(y_true_edges - y_pred_edges) 
    diff = smooth_l1_diff(diff)
    nocs_edge_loss = tf.reduce_mean(diff, axis=0)
    '''    
...
def mrcnn_coord_symmetry_loss_graph(target_masks, target_coords, target_class_ids, target_domain_labels, pred_coords, loss_fn):
    ...
    def nonzero_positive_loss(target_masks, target_coords, pred_coords, positive_ix):
        ...
        return loss, y_true1, y_pred1
    
    def zero_positive_loss():
        loss, y_true, y_pred = nonzero_positive_loss(target_masks, target_coords, pred_coords, positive_ix)
        return tf.constant([0.0, 0.0, 0.0]), y_true, y_pred


    loss, y_true, y_pred = tf.cond(tf.size(positive_ix) > 0,
...
def data_generator(dataset, config, shuffle=True, augment=False, random_rois=0,
    ...
    source_names = ['CAMERA', 'Real', 'coco', 'hand']
    weight = np.array([0, 0, 0 ,0], dtype=np.float32) 
    ...
    while True:
        try:
            ...
            source_ind = np.random.choice([0, 1, 2, 3], 1, p=weight)[0]
...
class MaskRCNN():
        ...
        if mode == "training":
            ...
            # direct regress
                ...
                if config.USE_SYMMETRY_LOSS:
                    ...
                    coord_loss, y_true, y_pred =  KL.Lambda(lambda x: mrcnn_coord_loss_graph(*x), name="mrcnn_coords_intm_loss")(
    ...
    def print_training_layers(self, layers_name):
        ...
        layer_regex = {
            #only changed layers
            "change": r"(mrcnn_mask_coord_x)|(mrcnn_mask_coord_y)|(mrcnn_mask_coord_z)|(mrcnn_bbox_fc)|(mrcnn_class_logits)|(mrcnn_mask)",
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers_name):
        ...
        layer_regex = {
            ...
            "mask": r"(mrcnn_mask_conv1)|(mrcnn_mask_bn1)|(mrcnn_mask_conv2)|(mrcnn_mask_bn2)|(mrcnn_mask_conv3)|(mrcnn_mask_bn3)|(mrcnn_mask_conv4)|(mrcnn_mask_bn4)|(mrcnn_mask_deconv)|(mrcnn_mask)",    
            # only coord map heads
            "coords": r"(mrcnn_coord\_.*)",
            # all layers but the backbone
            "heads": r"(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)",
    ...
    def detect(self, images, verbose=0):
        ...
        print('coords_shape........',final_coords.shape)  ################################################################    HERE
        results.append({
                "rois": final_rois,
