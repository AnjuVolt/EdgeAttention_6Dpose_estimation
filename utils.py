...
def minimize_mask(bbox, mask, mini_shape):
    ...
    if len(mask.shape)==3:
        ...
        for i in range(mask.shape[-1]):
            ...
            m = m[y1:y2, x1:x2]*255
            m = resize(m.astype(np.uint8), mini_shape, order=0)*255
    ...
    # for coordinate map
        ...
        for i in range(mask.shape[-2]):
            ...
            y1, x1, y2, x2 = bbox[i][:4]
            m = m[y1:y2, x1:x2, :]*255
            m = resize(m.astype(np.uint8), mini_shape+(mask.shape[-1],), order=0)*255
...
def unmold_mask(mask, bbox, image_shape):
    ...
    '''
    mask = resize(mask, (y2 - y1, x2 - x1), order=1).astype(np.float32) / 255.0
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    ...
    #print('15 degree, 10cm: {:.1f}'.format(pose_aps[-1, degree_thres_list.index(15),shift_thres_list.index(15)] * 100))
    '''
...
def draw(img, imgpts, axes, color, class_id, bbox):
    imgpts = np.int32(imgpts).reshape(-1, 2)
    '''
    w=sorted(imgpts[:4],key=lambda x: x[0])
    width1=w[1]-w[0]    
    width2=w[3]-w[2]
    
    w=sorted(imgpts[4:8],key=lambda x: x[0])   
    width3=w[1]-w[0]    
    width4=w[3]-w[2]
    
    #width1 = imgpts[2]-imgpts[0]
    #width2 = imgpts[3]-imgpts[1]
    #width3 = imgpts[6]-imgpts[4]
    #width4 = imgpts[7]-imgpts[5]
    imgpts[0]=np.array([bbox[0,1],bbox[0,0]])-width1/2
    imgpts[1]=np.array([bbox[0,3],bbox[0,0]])-width2/2
    imgpts[4]=np.array([bbox[0,1],bbox[0,2]])-width3/2
    imgpts[5]=np.array([bbox[0,3],bbox[0,2]])-width4/2
    imgpts[2]=imgpts[0]+width1
    imgpts[3]=imgpts[1]+width2
    imgpts[6]=imgpts[4]+width3
    imgpts[7]=imgpts[5]+width4
    '''
    ...
    cv2.putText(img, str(class_id) , (int(axes[0][0])-3 , int(axes[0][1]) ),
...
def draw_detections(image, save_dir, data_name, image_id, intrinsics, synset_names, draw_rgb_coord,
    ...
    if draw_gt:
        ...
        output_path_mask = os.path.join(save_dir, '{}_{}_mask_gt.png'.format(data_name, image_id))
        draw_image = image.copy()
        draw_image_mask = image.copy()
        ...
        for i in range(num_gt_instances):
            ...
            draw_image_mask[cind, rind, 0] = 255
            draw_image_mask[cind, rind, 1] = 255
            draw_image_mask[cind, rind, 2] = 0
        ...
        cv2.imwrite(output_path_mask, draw_image_mask[:, :, ::-1])
        ...
        if gt_RTs is not None:
            for ind, RT in enumerate(gt_RTs):
                ...
                draw_image_bbox = draw(draw_image_bbox, projected_bbox, projected_axes, (255, 0, 0), cls_id, gt_bbox)
    ...
    if draw_pred:
        ...
        output_path_mask   = os.path.join(save_dir, '{}_{}_mask_pred.png'.format(data_name, image_id))        
        ...
        for ind in range(num_pred_instances):
            ...
            transformed_bbox_3d = transform_coordinates_3d(bbox_3d, RT)
            projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
            draw_image_bbox = draw(draw_image_bbox, projected_bbox, projected_axes, (255, 0, 0),cls_id,pred_bbox)
...
def backproject(depth, intrinsics, instance_mask):
    ...
    '''
    pts_anju=np.array([np.transpose(xyz)[0],np.transpose(xyz)[1],z]) #################NEW By Anju
    pts_anju=np.transpose(pts_anju) ##################################################NEW By Anju
    ...
    pts[:, 1] = -pts[:, 1]
    '''
    pts_anju2 = np.array([uv_grid[0],uv_grid[1],z])
    pts_anju2 = intrinsics_inv @ pts_anju2
    pts_anju2 = np.transpose(pts_anju2)
    '''
    return pts, idxs


def align(class_ids, masks, coords, depth, intrinsics, synset_names, image_path, save_path=None, verbose=False, if_norm=False, with_scale=True):
    ...
    Rotation = np.zeros((num_instances, 3, 3))
    Translation = np.zeros((num_instances, 3))
    Scales = np.zeros((num_instances, 3))
    ...
    for i in range(num_instances):
        ...
        pts, idxs = backproject(depth, intrinsics, mask)        # pts is the 3d point cloud from instance mask and depth image
        coord_pts = coord[idxs[0], idxs[1], :] - 0.5            # coord_pts is the 3d point cloud from nocs map
        if if_norm:
        ...
        try:
            start = time.time()
            # Function that finds rotation, translation scale (Umeyama Algorithm)
        ...
        Rotation[i] = z_180_RT[:3,:3] @ rotation.transpose()
        Translation[i] = translation/1000
        Scales[i] = scales/1000

        RTs[i, :, :] = z_180_RT @ aligned_RT 
        
    return RTs, bbox_scales, error_messages, elapses, Rotation, Translation
...
def align_ICP(class_ids, masks, depth, intrinsics, synset_names, image_path, save_path=None, if_norm=False, verbose=False):
    ...
    for i in range(num_instances):
        ...
        glob_paths = '/home/anju/Anju/FINAL_yr_project/NOCS_CVPR2019-master1/data/obj_models/real_test/{}*.txt'.format(synset_names[class_ids[i]])
        print(glob_paths)
