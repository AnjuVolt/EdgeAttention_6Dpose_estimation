...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 05:07:59 2020
...
def load_weights_from_hdf5_group_by_name(f, layers, skip_mismatch=False,
    ...
    for k, name in enumerate(layer_names):
        ...
        for layer in index.get(name, []):
            ...
            #x = weight_values[269]  
            #print(x.shape)
            #mat_to_be_concat = (x[:,0:4]+x[:,4:8]+x[:,8:12]+x[:,12:16]+x[:,16:20]+x[:,20:24]+x[:,24:28])/7  
            #weight_values[269] = np.concatenate((weight_values[269],mat_to_be_concat),axis=1)
            #print(weight_values[269].shape)
            if len(weight_values) != len(symbolic_weights):
                if skip_mismatch:
                    ...
                    raise ValueError('Layer #' + str(k) +
                                     ...
                                     ' element(s).')
            # Set values.
            #print('len(weight_values)',len(weight_values))
            for i in range(len(weight_values)):
                symbolic_shape = K.int_shape(symbolic_weights[i])
                if symbolic_shape != weight_values[i].shape:
                    print('shape_mismatch',i,weight_values[i].shape)
                    print('symbolic_shape',symbolic_shape)
            for i in range(len(weight_values)):
                symbolic_shape = K.int_shape(symbolic_weights[i])
                if symbolic_shape != weight_values[i].shape:
                    if weight_values[i].shape==(1024,28):
                        print('i',i)
                        x = weight_values[i]  
                        print('weight_vales[i]',x.shape)
                        mat_to_be_concat=np.zeros((1024,4))
                        mat_to_be_concat = (x[:,0:4]+x[:,4:8]+x[:,8:12]+x[:,12:16]+x[:,16:20]+x[:,20:24]+x[:,24:28])/7
                        weight_values[i] = np.concatenate((weight_values[i],mat_to_be_concat),axis=1)
                        print('after',weight_values[i].shape)
                        weight_value_tuples.append((symbolic_weights[i],
                                                weight_values[i]))
                    elif weight_values[i].shape==(28,):
                        print('i',i)
                        x1 = weight_values[i]  
                        print('weight_vales[i]',x1.shape)
                        mat_to_be_concat1=np.zeros(4)
                        mat_to_be_concat1 = (x1[0:4]+x1[4:8]+x1[8:12]+x1[12:16]+x1[16:20]+x1[20:24]+x1[24:28])/7
                        print('mat',mat_to_be_concat1.shape)
                        weight_values[i] = np.concatenate((weight_values[i],mat_to_be_concat1),axis=0)
                        print('after',weight_values[i].shape)
                        weight_value_tuples.append((symbolic_weights[i],
                                                weight_values[i]))
                    elif weight_values[i].shape==(1024,7):
                        print('i',i)
                        x1 = weight_values[i]  
                        print('weight_vales[i]',x1.shape)
                        mat_to_be_concat1=np.zeros((1024,1))
                        mat_to_be_concat1 = (x1[:,0]+x1[:,1]+x1[:,2]+x1[:,3]+x1[:,4]+x1[:,5]+x1[:,6])/7
                        mat_to_be_concat1 = mat_to_be_concat1.reshape((1024,1))
                        print('mat',mat_to_be_concat1.shape)
                        weight_values[i] = np.concatenate((weight_values[i],mat_to_be_concat1),axis=1)
                        print('after',weight_values[i].shape)
                        weight_value_tuples.append((symbolic_weights[i],
                                                weight_values[i]))    
                    elif weight_values[i].shape==(7,):
                        print('i',i)
                        x1 = weight_values[i]  
                        print('weight_vales[i]',x1.shape)
                        #mat_to_be_concat1=np.zeros(4)
                        mat_to_be_concat1 = np.mean(x1)
                        print('mat',mat_to_be_concat1.shape)
                        weight_values[i] = np.concatenate((weight_values[i],mat_to_be_concat1.reshape(1)),axis=0)
                        print('after',weight_values[i].shape)
                        weight_value_tuples.append((symbolic_weights[i],
                                                weight_values[i]))  
                     
                    elif symbolic_shape==(1,1,256,8):
                        print('i',i)
                        x1 = weight_values[i]  
                        print('weight_vales[i]',x1.shape)
                        mat_to_be_concat1=np.zeros((((1,256,1,1))))
                        #x1=np.transpose(x1,(3,2,1,0))
                        print('x1',x1.shape)
                        mat_to_be_concat1 = (x1[0,:,:,:]+x1[1,:,:,:]+x1[2,:,:,:]+x1[3,:,:,:]+x1[4,:,:,:]+x1[5,:,:,:]+x1[6,:,:,:])/7
                        mat_to_be_concat1=mat_to_be_concat1.reshape(1,256,1,1)
                        print('mat',mat_to_be_concat1.shape)
                        weight_values[i] = np.transpose(np.concatenate((weight_values[i],mat_to_be_concat1),axis=0),(3,2,1,0))
                        print('after',weight_values[i].shape)
                        weight_value_tuples.append((symbolic_weights[i],
                                                weight_values[i]))     
                    else: 
